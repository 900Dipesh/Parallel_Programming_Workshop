...
__global__ void arradd(double* global_area)
{
	...
	if(myid < N)
	{
		...
		active_thds = THDS_PER_BLK /2;
		while(active_thds > 0)
		{
			if(threadIdx.x < active_thds)
			{
				tmp_area[threadIdx.x] += tmp_area[threadIdx.x + active_thds]; 
			}
			active_thds = active_thds / 2;
			__syncthreads();
		}
		if(threadIdx.x == 0)
		{
			tmp = atomicAdd(&global_area[0],tmp_area[0]);
		}
	}
}
